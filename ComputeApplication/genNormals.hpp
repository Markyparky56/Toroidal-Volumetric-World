...
// More sane Vertex specialised version
inline void generateNormals(
  Vertex * vertices, const unsigned int vertexCount
  , const unsigned int * indices, const unsigned int indexCount)
{
  // First, 0 all normals
  for (unsigned int i = 0; i < vertexCount; i++)
  ...
  for (unsigned int i = 0; i < indexCount; i += 3)
  {
    ...
    const glm::vec3 * va = &vertices[ia].pos;
    const glm::vec3 * vb = &vertices[ib].pos;
    const glm::vec3 * vc = &vertices[ic].pos;

    const glm::vec3 v1 = *va - *vb;
    const glm::vec3 v2 = *vc - *vb;
    const glm::vec3 normal = glm::cross(v1, v2);

    glm::vec3 * na = &vertices[ia].normal
             , *nb = &vertices[ib].normal
             , *nc = &vertices[ic].normal;
